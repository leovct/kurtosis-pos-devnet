...
def generate_config_and_scripts(
    ...
    start_script = read_file("./config/start.sh")
    return plan.render_templates(
        ...
        config={
            ...
            "scripts/start.sh": struct(
                ...
                data={
                    "NODE_ID": id,
                    "VALIDATOR_NODE_PRIVATE_KEY": validator_private_key,
                    "CHAIN_ID": CHAIN_ID,
                    "DATA_PATH": DATA_PATH,
                    "RABBITMQ_AMQP_URL": rabbitmq_amqp_url,
                },
...
def start(plan, id, config):
    """
    Start a Heimdall node.

    Args:
        id (string): The unique identifier for the node.
        config (str): The configuration files and start script.
